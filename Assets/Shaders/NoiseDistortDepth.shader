...
Shader "Unlit/NoiseDistortDepth"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        _NormalsTex ("Texture", 2D) = "white" {}
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 GetViewVec(float4 bottomLeft, float4 topLeft, float4 bottomRight, float4 topRight, float2 uv){
                const float4 leftLerp = lerp(bottomLeft, topLeft, uv.y);
                const float4 rightLerp = lerp(bottomRight, topRight, uv.y);
                float4 viewVec = lerp(leftLerp, rightLerp, uv.x);
            ...
            float4 AddNoise(float4 color, float2 uv, float intensity){
                float2 sample = GaussianRandom(uv);
                float noise = clamp(sample.x, -1.0, 1.0)*intensity;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float4 normal = tex2D(_NormalsTex, xy_u);
                const float4 viewVec = GetViewVec(_BL, _TL, _BR, _TR, xy_u);
